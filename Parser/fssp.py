...
get_params = {'token': 'token', 'region': region, 'firstname': firstname, 'lastname': lastname, 'birthdate': birthdate}
...
print(val[0])
v = str(val[0])
status = requests.get('https://api-ip.fssp.gov.ru/api/v1.0/status', params={'token': 'token', 'task': v})
...
while s != 0:
    ...
    st = list(sta[3].values())
    s = int(st[0])
result = requests.get('https://api-ip.fssp.gov.ru/api/v1.0/result', params={'token': 'token', 'task': v})
print(result.json())
